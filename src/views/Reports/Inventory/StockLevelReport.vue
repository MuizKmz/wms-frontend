        ...
        <template #a4-content>
          <div id="printable-report">
            ...
            <div v-if="savedDesign?.components && savedDesign.components.length > 0" class="absolute inset-0 pointer-events-none">
              ...
              >
                <!-- Signature Component -->
                <div v-if="component.type === 'signature'" class="text-center bg-white p-2">
                  <p class="text-xs text-gray-600 mb-2">{{ component.data.label }}</p>
                  <img v-if="component.data.image" :src="component.data.image" alt="Signature" class="max-w-full h-16 mx-auto" />
                  <div v-else class="border-b-2 border-gray-800 h-8"></div>
                  <p class="text-xs text-gray-500 mt-1">Signature</p>
                </div>

                <!-- Stamp Component -->
                <div v-else-if="component.type === 'stamp'" class="flex justify-center bg-white p-2">
                  <img v-if="component.data.image" :src="component.data.image" alt="Stamp" class="w-20 h-20 object-contain" />
                ...
                <!-- Text Component -->
                <div v-else-if="component.type === 'text'" class="bg-white p-2">
...
const exportToPDF = () => {
  ...
  pdf.text('Stock Level Report', 105, 20, { align: 'center' })
  
  pdf.setFontSize(10)
  pdf.setFont('helvetica', 'normal')
  pdf.text('Inventory Status Across All Locations', 105, 28, { align: 'center' })
  pdf.text(`Generated: ${reportDate.value.toLocaleDateString()}`, 105, 35, { align: 'center' })
  ...
  if (filters.length > 0) {
    pdf.setFontSize(9)
    pdf.text('Filters Applied:', 14, yPos)
    yPos += 5
    filters.forEach(filter => {
      pdf.text(`${filter.label}: ${filter.value}`, 20, yPos)
      yPos += 5
    })
    yPos += 5
  }
  
  // Add summary cards if visible
  if (showSummaryCards.value) {
    const cards = getSummaryCards()
    pdf.setFontSize(9)
    pdf.setFont('helvetica', 'bold')
    let xPos = 14
    cards.forEach(card => {
      pdf.text(`${card.label}: ${card.value}`, xPos, yPos)
      xPos += 60
    })
    yPos += 10
  }
  ...
  autoTable(pdf, {
    startY: yPos,
    head: [['Product Name', 'Warehouse', 'Current Stock', 'Min Level', 'Status']],
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    pdf.text(companyName.value, 14, 285)
    pdf.text(`Page ${i} of ${pageCount}`, 105, 285, { align: 'center' })
    pdf.text(`Generated by: Admin User`, 196, 285, { align: 'right' })
  }
  
  pdf.save(`stock-level-report-${new Date().getTime()}.pdf`)
