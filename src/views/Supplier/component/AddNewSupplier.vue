...
const validateForm = () => {
  ...
  if (!form.picName.trim()) {
    ...
    isValid = false
  } else if (form.picName.length < 2) {
    errors.picName = 'PIC Name must be at least 2 characters'
    isValid = false
  } else if (form.picName.length > 50) {
    errors.picName = 'PIC Name cannot exceed 50 characters'
    isValid = false
  }
  ...
  if (!form.phoneNumber.trim()) {
    ...
    errors.contact = 'Contact Number cannot exceed 10 digits'
    isValid = false
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    const submissionData = {
      ...form,
      contact: `${form.countryCode}${form.phoneNumber}` // Combine country code + phone number
    }
