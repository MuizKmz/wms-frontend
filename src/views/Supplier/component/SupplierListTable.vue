...
const filteredData = computed(() => {
  ...
  return data.value.filter((item) => {
    ...
    if (filters.picName && !picName.toLowerCase().includes(filters.picName.toLowerCase())) {
      return false
    }
...
const deleteSupplier = async (supplier) => {
  ...
  try {
    ...
    emit('delete-supplier', {
      success: true,
      data: { supplierName: supplier.supplierName, id: supplier.id },
    })
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (deletedCount > 0 && blocked.length === 0) {
      ...
      const message =
        blocked.length > 0
          ? `${blocked.length} suppliers were blocked from deletion due to existing relations.`
