            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl flex flex-col">
              ...
              <div class="space-y-6 overflow-y-auto p-6 flex-1 text-center">
                ...
                <p class="text-sm font-semibold text-gray-700 dark:text-gray-300 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                    EPC Code: <span class="text-gray-900 dark:text-white font-normal break-all">{{ displayedEpcCode || 'N/A' }}</span>
                </p>
...
const props = defineProps<{
  show: boolean; // Controls modal visibility from parent
  initialEpcCode: string | null; // The EPC code to display
}>();
...
const displayedEpcCode = ref(props.initialEpcCode || '');
const isQRCode = ref(true); // true for QR Code, false for Bar Code
const codeImageUrl = ref<string | null>(null); // Stores the fetched image URL
...
const fetchCodeImage = async () => {
    const code = displayedEpcCode.value;
    if (!code) {
        imageError.value = "No EPC code provided.";
...
// ------------------------------------------------
// --- Scroll Lock Utilities (Keeping existing logic) ---
// ------------------------------------------------
let scrollY = 0;
...
const openModal = async (code: string | null) => {
    displayedEpcCode.value = code || '';
    codeImageUrl.value = null;
    imageError.value = null;
    loading.value = false;
...
const downloadCode = () => {
    ...
    link.download = `${displayedEpcCode.value}-${isQRCode.value ? 'QR' : 'BAR'}.png`;
...
const printCode = () => {
    ...
    if (printWindow) {
        ...
        printWindow.document.write(`<p style="font-family: Arial, sans-serif; font-size: 14px; text-align: center;">EPC Code: ${displayedEpcCode.value}</p>`);
...
watch(() => props.initialEpcCode, (newCode) => {
    if (isOpen.value) {
        displayedEpcCode.value = newCode || '';
        fetchCodeImage();
    }
});
...
// EXPOSE the required methods for the parent component to call
defineExpose({ openModal, closeModal });
</script>

<style scoped>
/* Ensure the inner content animates from the center */
