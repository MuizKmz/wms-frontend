          ...
          <div v-if="isOpen" ref="panelRef" class="relative z-10 w-full max-w-xl mx-4 max-h-[90vh] flex flex-col" @click.stop role="dialog" aria-modal="true" aria-labelledby="modal-title">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl flex flex-col min-h-0 max-h-[90vh]">
              <div class="flex items-center justify-between p-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                <h2 id="modal-title" class="text-lg font-semibold text-gray-900 dark:text-white">Add Location Information</h2>
              ...
              <div class="flex justify-end gap-2 p-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button @click="submitForm" class="btn bg-brand-500" :disabled="isSubmitting"><span v-if="isSubmitting" class="loading loading-spinner loading-sm mr-2"></span>{{ isSubmitting ? 'Saving...' : 'Save' }}</button>
              </div>
            </div>
          </div>
        </transition>
...
const submitForm = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      let errorMessage = `Failed to create location (HTTP ${response.status}).`
      try {
    ...
    errors.submit = error.message || 'An unknown error occurred. Failed to create location.'
    emit('item-created', { success: false, error: error.message || 'Failed to create location' })
  } finally {
