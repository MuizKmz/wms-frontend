                ...
                <div class="relative">
                  ...
                  <div class="dropdown relative inline-flex w-full" ref="parentCategoryDropdownRef">
                    ...
                    >
                      ...
                      <li v-for="category in filteredParentCategories" :key="category.id">
...
const openModal = async (category: Category) => {
  ...
  form.storageRequirements = category.storageRequirements || ''
  form.status = category.status

  // Set parent category name for display
  if (category.parentCategoryId) {
    const parent = parentCategories.value.find(c => c.id === category.parentCategoryId)
    selectedParentCategoryName.value = parent ? parent.name : ''
  } else {
    selectedParentCategoryName.value = 'None (Top Level)'
  }
...
const submitForm = async () => {
  if (!validateForm() || !currentCategoryId.value) {
    return
  }
  ...
  try {
    ...
    const submissionData = {
      ...
      level: form.parentCategoryId ? 2 : 1,
    ...
    emit('category-updated', { success: true, data })
    ...
    emit('category-updated', { success: false, error: errorMessage })
  } finally {
