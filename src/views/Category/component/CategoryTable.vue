...
const visibleCategoryRows = computed(() => {
  ...
  if (areFiltersActive.value) {
    ...
    while (i >= 0) {
      ...
      const shouldShow =
        matchingIds.has(row.id) || visibleRows.some((vr) => vr.parentCategoryId === row.id)
...
watch([paginatedTopLevelData, expandedCategories], updateSelectAllState, {
  deep: true,
  immediate: true,
})
...
const deleteCategory = async (item) => {
  ...
  try {
    ...
    const deletedIds =
      body && Array.isArray(body.deletedIds) && body.deletedIds.length ? body.deletedIds : [item.id]
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (deletedCount > 0 && blocked.length === 0) {
      ...
      const message =
        blocked.length > 0
          ? `${blocked.length} categories were blocked from deletion due to existing products.`
