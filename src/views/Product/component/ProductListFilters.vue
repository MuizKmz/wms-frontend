    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      <div class="dropdown relative inline-flex w-full">
        ...
        <ul
          ...
          :class="{
            'opacity-100': openDropdowns.category,
            'opacity-0 pointer-events-none': !openDropdowns.category,
          }"
          role="menu"
          aria-orientation="vertical"
        >
          <li v-if="loadingCategories" class="px-4 py-2 text-sm text-gray-500">
            Loading categories...
          </li>
          <li>
            <a
          ...
          <li v-for="category in categories" :key="category.value">
            <a
              class="block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer rounded-lg"
              @click="selectOption('category', category.value)"
            >
              {{ category.label }}
            </a>
          </li>
...
const fetchCategories = async () => {
  ...
  try {
    ...
    categories.value = (data || []).map((item) => ({
      label: item.name || item.categoryName || item.categoryCode,
      value: item.name || item.categoryName || item.categoryCode,
    }))
  } catch (error) {
    ...
    console.log('Final categories list:', categories.value) // Debug log for processed categories
  }
}
