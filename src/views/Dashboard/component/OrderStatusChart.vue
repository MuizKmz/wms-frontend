  ...
  <div class="w-full h-full relative flex flex-col overflow-visible" ref="cardContainer">
    ...
    <div class="flex justify-end mb-3 flex-shrink-0">
      <div class="dropdown relative inline-flex">
        ...
        <ul
          class="dropdown-menu w-[160px] transition-opacity duration-200 absolute top-full right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg text-gray-900 dark:text-white"
          :class="{
            'opacity-100 visible z-[9999]': isDropdownOpen,
...
const handleClickOutside = (event) => {
  if (
    rangeDropdownRef.value &&
    !rangeDropdownRef.value.closest('.dropdown').contains(event.target)
  ) {
    isDropdownOpen.value = false
  }
}
...
const fetchData = async () => {
  ...
  try {
    ...
    const statusCounts = Object.fromEntries(allStatuses.map((status) => [status, 0]))
    ...
    // Use a consistent label order
    const labels = allStatuses
    const data = allStatuses.map((status) => statusCounts[status])
    ...
    createChart({
      labels: mock.labels,
      data: mock.datasets[0].data,
      colors: mock.datasets[0].backgroundColor,
    })
  } finally {
    loading.value = false
  }
